...
static bool append_hardcore_stats_death_event(std::shared_ptr<Client> c, const char* reason);
...
static bool mark_bb_character_hardcore_dead_for_subcommands(std::shared_ptr<Client> c, const char* reason) {
  ...
  if (bb_hardcore_dead_marker_matches_current_character_for_subcommands(c, filename)) {
    return false;
  }
  ...
  if (ok) {
    append_hardcore_stats_death_event(c, reason);
  }
  return ok;
}
...
static bool append_hardcore_stats_enemy_kill_event(
  ...
  try {
    ...
    {
      std::ifstream hardcore_f(c->character_filename() + ".hardcore");
      if (!hardcore_f.good()) {
        return true;
      }
    }

    std::ofstream f("system/hardcore-stats/events.jsonl", std::ios::out | std::ios::app);
    if (!f.good()) {
...
static bool append_hardcore_stats_death_event(std::shared_ptr<Client> c, const char* reason) {
  try {
    ...
    std::string location = hardcore_zone_name(c);
...
static bool bb_hardcore_dead_marker_matches_current_character_for_subcommands(
  ...
  while (getline(f, line)) {
    const std::string prefix = "creation_timestamp=";
...
static void on_ep3_battle_subs(std::shared_ptr<Client> c, SubcommandMessage& msg) {
  ...
  for (const auto& lc : l->clients) {
    ...
    if (!(s->data->ep3_behavior_flags & Episode3::BehaviorFlag::DISABLE_MASKING) && (lc->version() != Version::GC_EP3_NTE)) {
    ...
    if ((room >= 0) && (wave_number >= 0)) {
      ...
      if (all_enemies_defeated) {
        ...
        for (auto ev_st : l->map_state->event_states_for_floor_room_wave(c->version(), cmd.floor, room, wave_number)) {
          bool already_finished = (ev_st->flags & 0x10);
...
static void on_upgrade_weapon_attribute_bb(std::shared_ptr<Client> c, SubcommandMessage& msg) {
  ...
  try {
    ...
    p->remove_item(payment_item.id, cmd.payment_count, *s->data->item_stack_limits(c->version()));
