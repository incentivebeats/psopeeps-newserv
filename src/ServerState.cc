...
std::shared_ptr<ServerState> ServerState::create_shared(std::shared_ptr<DataIndex> data, bool is_replay) {
  ...
  AccountSync::configure_from_json(s->data->config_json->get("AccountSync", phosg::JSON::dict()));
  AccountSync::start_login_lock_heartbeat_task(*s->io_context);
  TeamSync::configure_from_json(s->data->config_json->get("TeamSync", phosg::JSON::dict()));
  TeamSync::start_exchange_task(*s->io_context);
